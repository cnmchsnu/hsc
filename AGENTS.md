...
Code generated by `pnpm dlx shadcn@latest add` must not be considered production-ready by default.

After installation, agents must review the generated component to ensure it:

- Conforms to the project's coding standards.
- Uses the project's import aliases.
- Integrates with the shared UI package structure.
- Matches the project's design tokens.
- Removes unused utilities or dependencies.
- Exports through the shared package entry points.