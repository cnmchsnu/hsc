...
import { ProductDetail } from "../../../../../../../packages/commerce/application/read";


export function SkuSelector({productData}: {productData: ProductDetail}) {
    ...
    return (
        <div className="space-y-6 mb-stack-lg">
          {variant.options.find((opt) => opt.option.name === "color") && (
            <div>
              <span className="block font-bold text-on-surface mb-3">選擇顏色</span>
              <div className="flex gap-3">
                {variant.options.find((opt) => opt.option.name === "color")?.values.map((c, idx) => (
                  <button
                    key={idx}
          ...
          {variant.options.find((opt) => opt.option.name === "size") && (
            <div>
              ...
              <div className="flex gap-3">
                {variant.options.find((opt) => opt.option.name === "size")?.values.map((s) => (
                  <button
                    key={s.value_name}
                    onClick={() => setSelectedSize(s.value_name)}
                    className={`w-12 h-12 flex items-center justify-center border-2 rounded-lg transition-all font-label-md ${
                      selectedSize === s.value_name
